...
export interface TareasContextProps {
  ...
  editarTarea: (id_Tarea: number, nuevaTarea: TareasTypeSchema) => Promise<void>;
  borrarTareas: (id_Tarea: number) => Promise<void>;
  getTarea: (id_Tarea: number) => Promise<void>;
...
export const TareasProvider: FC<ProviderProps> = ({ children }) => {
  ...
  const getTarea = async (id_Tarea: number) => {
    try {
      const respuesta = await getTareaRequest(id_Tarea);
  ...
  const agregarTarea = async (tarea: TareasTypeSchema) => {
    try {
  ...
  const editarTarea = async (id_Tarea: number, tarea: TareasTypeSchema) => {
    try {
      const respuesta = await actualizarTareaRequest(id_Tarea, tarea);
  ...
  const borrarTareas = async (id_Tarea: number) => {
    try {
